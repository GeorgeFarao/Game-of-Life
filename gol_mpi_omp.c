...
char **Make_grid(int size, int value_flag, char value) {
    char **temp, *arrDt;
...
int main(int argc, char *argv[]) {
    if (argc !=
        9) {  //mpirun -np (processes) mpi_prog -g (generations) -sz (N for N*N grid) -sc (sanity 0 or 1) -th thread_num
    ...
    if (strcmp(argv[3], "-sz") == 0) {
        arr_size = atoi(argv[4]);
    }
    if (strcmp(argv[5], "-sc") == 0) {
        sanity = atoi(argv[6]);
    }

    if (strcmp(argv[7], "-th") == 0) {
    ...
    char **recv_array = malloc(8 * sizeof(char *));
    for (int l = 0; l < 8; ++l) {
    ...
    for (int m = 0; m < gens; ++m) {
        ...
        int neighbours;
        int k, l;
# pragma omp parallel num_threads(thread_num) shared(block, next, block_ar_len, zeros, same) private(k, l, n, s, w, e, neighbours)
        {
#pragma omp for
            for (k = 1; k < block_ar_len - 1; ++k) {
                n = k - 1;
                s = k + 1;
                for (l = 1; l < block_ar_len - 1; ++l) {
                    neighbours = 0;
                    w = l - 1;
                    e = l + 1;
        ...
        //upper line
        int i, j;
#pragma omp parallel num_threads(thread_num) shared(block, next, recv_array, zeros, same, block_ar_len) private(j, neighbours)
        {
#pragma omp for
            for (j = 0; j < block_ar_len; ++j) {
                neighbours = 0;
                if (j == 0) {  //left corner
                    if (block[0][j + 1] == 1)
                        neighbours++;
...
#pragma omp parallel num_threads(thread_num) shared(block, next, recv_array, zeros, same, block_ar_len) private(i, neighbours)
        {
#pragma omp for
            //in recv_array: north=0, south=1, east=2, west=3, ne=4, nw=5, se=6, sw=7
            //left column, corners are already finished
            for (i = 1; i < block_ar_len - 1; ++i) {
                neighbours = 0;
...
#pragma omp for
    ...
    MPI_Gatherv(&(block[0][0]), arr_size * arr_size / comm_sz, MPI_CHAR, &(current_arr[0][0]), num, begin, block_type,
                0, MPI_COMM_WORLD);
    MPI_Pcontrol(0);
    end_time = MPI_Wtime();
    ...
    MPI_Reduce(&total_time, &max, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    MPI_Reduce(&total_time, &min, 1, MPI_DOUBLE, MPI_MIN, 0, MPI_COMM_WORLD);
    MPI_Reduce(&total_time, &average, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
